"""
    Tests for the :mod:`regression_tests.tools.unpacker_arguments` module.
"""
...
class UnpackerArgumentsTests(unittest.TestCase):
    """Tests for `UnpackerArguments`."""
    ...
    def test_from_test_settings_args_is_present_when_set(self):
        test_settings = UnpackerTestSettings(input='test.exe', args='--arg1 --arg2')
        args = UnpackerArguments.from_test_settings(test_settings)
    ...
    def test_from_test_settings_error_is_raised_when_there_is_no_input(self):
        test_settings = UnpackerTestSettings(input=None)
    ...
    def test_from_test_settings_error_is_raised_when_input_is_list(self):
        test_settings = UnpackerTestSettings(input=['test1.exe', 'test2.exe'])
        self.scenario_invalid_settings_error_is_raised(test_settings, 'input')
    ...
    def test_without_paths_and_output_files_returns_correct_args_when_there_are_files(self):
        args = UnpackerArguments(
            ...
            output_file=File('file.exe-unpacked', Directory(os.path.join(ROOT_DIR, 'outputs')))
        )
